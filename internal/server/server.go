...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/policys", s.list)
	s.mux.HandleFunc("POST /api/policys", s.create)
	s.mux.HandleFunc("GET /api/policys/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/policys/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-covenant", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"policys": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Policy{} }
	wj(w, map[string]any{"policys": items, "count": len(items)})
}
